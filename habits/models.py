...
class Habit(models.Model):
    ...
    frequency = models.PositiveIntegerField(
        default=1, verbose_name="Частота повторения", help_text="Периодичность в днях"
    )  # раз в N дней
    ...
    def clean(self):
        ...
        if self.related_habit:
            ...
            if self.related_habit.owner != self.owner:
                errors["related_habit"] = (
                    "Можно привязывать только свои приятные привычки."
                )
    ...
    def is_due_now(self, minutes_interval: int = 5) -> bool:
        ...
        habit_datetime = timezone.make_aware(datetime.combine(today, self.time))
